...
import net.yazibacloud.android.lib.resources.shares.GetRemoteShareesOperation;
...
public class UsersAndGroupsSearchProvider extends ContentProvider {
     ...
     *                      "content://UsersAndGroupsSearchProvider/" +
            ...
            try {
                while (namesIt.hasNext()) {
                    ...
                    if (ShareType.GROUP.getValue() == type) {
                        displayName = getContext().getString(net.yazibacloud.android.R.string.share_group_clarification, userName);
                        dataUri = Uri.withAppendedPath(groupBaseUri, shareWith);
                    } else if (ShareType.FEDERATED.getValue() == type && federatedShareAllowed) {
                        if (userName.equals(shareWith)) {
                            displayName = getContext().getString(net.yazibacloud.android.R.string.share_remote_clarification, userName);
                        } else {
                            String[] uriSplitted = shareWith.split("@");
                            displayName = getContext().getString(net.yazibacloud.android.R.string.share_known_remote_clarification, userName,
